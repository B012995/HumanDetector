...
# 学習用画像ディレクトリと検証用画像ディレクトリを指定
train_dir = "/Users/nttcom/Desktop/DataSet/TrainDataSet"
...
flags.DEFINE_string('tensorflow_dir', '/Users/nttcom/Desktop/TensorFlowData', 'Directory to put the training data.')
# 学習訓練の試行回数
...
def inference(images_placeholder, keep_prob):
    ...
    with tf.name_scope('fc2') as scope:
        W_fc2 = weight_variable([1024, len(tag_name)])
        b_fc2 = bias_variable([len(tag_name)])
...
if __name__ == '__main__':
    ...
    # 学習用画像ファイル名をlistに格納
    train_file_list = sorted(glob.glob(train_dir + "*"))

    # 画像,ラベルをそれぞれ一つにまとめて格納するlistを用意
    train_image = []
    train_label = []

    # 学習用画像をTensorFlowで読み込めるようTensor形式に変換
    for train_filename in train_file_list:
        # 画像を読み込む
        img = cv2.imread(train_dir + train_filename)
        # 画像をリサイズ
        img = cv2.resize(img, (img_size, img_size))
        # 画像のタグを取得する
        tag = train_filename.split("-")[0]

        # ラベルを1-of-k方式で用意する
        tmp = np.zeros(len(tag_name))
        try:
            tmp[tag_name.index(tag)] = 1
        except:
            print("タグ名が見つかりません。処理をスキップします。")
        else:
            # タグlistを追加する
            train_label.append(tmp)
            # 一列にした後、0-1のfloat値にして画像listを追加する
    ...
    # 検証用画像ファイル名をlistに格納
    test_file_list = sorted(glob.glob(test_dir + "*"))

    # 画像,ラベルをそれぞれ一つにまとめて格納するlistを用意
    test_image = []
    test_label = []

    # 検証用画像をTensorFlowで読み込めるようTensor形式に変換
    for test_filename in test_file_list:
        # 画像を読み込む
        img = cv2.imread(test_dir + test_filename)
        # 画像をリサイズ
        img = cv2.resize(img, (img_size, img_size))
        # 画像のタグを取得する
        tag = test_filename.split("-")[0]
    ...
    with tf.Graph().as_default():
        # 画像を入れるためのTensor(img_pixel次元の画像が任意の枚数(None)分入る)
        images_placeholder = tf.placeholder("float", shape=(None, img_pixel))
        # ラベルを入れるためのTensor(len(tag_name)次元のラベルが任意の枚数(None)分入る)
        labels_placeholder = tf.placeholder("float", shape=(None, len(tag_name)))
